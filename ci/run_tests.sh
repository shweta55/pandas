...
if [[ "$TRAVIS_OS_NAME" == "linux" ]]; then
    ...
    if [ `uname -m` = 'aarch64' ]; then
        PYTEST_CMD="xvfb-run -e /dev/stdout pytest --ignore=pandas/tests/io/parser -m \"$PATTERN\" -s --strict --durations=10 --junitxml=test-data.xml $TEST_ARGS $COVERAGE pandas"
    else
